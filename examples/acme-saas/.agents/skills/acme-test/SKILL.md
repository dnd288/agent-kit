...
`acme-tdd` owns *writing* tests — the mode-to-claim mapping and the RED→GREEN discipline, and
`acme-e2e` owns the end-to-end flow itself: how a change's scenarios become a Gherkin feature
...
serialisation via `app.inject()` — see `acme-tdd`. For what an end-to-end flow should look
like, or what a failing one means once the stack is up, see `acme-e2e`. This skill is the
"run it" half.
...
same in `acme-e2e` — version-matched on purpose. Never `pnpm exec playwright` at the root: it resolves
...
This is how the stack comes up. `acme-e2e` is how a flow is written against it and how a red
...
unfinished flow rather than a red one — `acme-e2e` owns the fix. The generated tree under
...
1. **Create an isolated database.** On the Postgres `DATABASE_URL` points at, create
   ...
   DATABASE_URL="<app-postgres>/project_e2e" pnpm --filter your-data-layer db:migrate:deploy
   DATABASE_URL="<app-postgres>/project_e2e" pnpm db:seed
   ```
...
- [ ] A red flow got a verdict (`acme-e2e`) before the environment was blamed
