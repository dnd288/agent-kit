...
This skill is the middle of three. `acme-tdd` decides **whether** a claim belongs in this mode;
`acme-test` brings the stack up and owns every environment gotcha. This one owns what happens in
...
| Question | Owner |
|---|---|
| Bringing the stack up — the isolated e2e database, the rate limit, `reuseExistingServer` — and how much of the suite one run selects | `acme-test`; the selectors are your e2e README's *Running a slice* |
| Whether this claim needs `acme-e2e` at all, and the RED→GREEN order | `acme-tdd` |
...
| Everything after a defect is confirmed — diagnosis, change folder, fix | `acme-bugfix` |
...
- **Assert the seam, not the unit.** If the assertion would hold with the other process stubbed,
  it belongs one layer down and `acme-tdd` says where.
...
because the application is *wrong* is a defect and belongs to `acme-bugfix`. And it is skipped
...
performing it yourself does `acme-bugfix`'s work out of order.
...
| **Environment** | Every scenario in the file fails identically, or generation fails outright — `acme-test` owns it |
| **A defect** | Everything else is excluded and the application is genuinely wrong — hand off to `acme-bugfix` |
