...
widgets the screen has not been split into. `acme-ui-development` §9 owns the split itself.
...
- `acme-app-development` §7a — the same ladder from the Next side, with the mechanics.
- `acme-api-contract` — the wire the values cross.
