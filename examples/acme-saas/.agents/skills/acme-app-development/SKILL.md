...
your project's architectural decisions; the visual language is `acme-design`.
...
or `page.tsx` to make a child work; push it down. `acme-component-development` covers which component files get
...
half out (`members/people-tab.ts`). Only e2e can observe this — see `acme-tdd` for why.
...
`docs/design/design-system.md` specifies `next/font`, while `acme-design`'s `tokens/fonts.css`
...
component, one-way arrows, and why each rule exists — is the **`acme-state-management` skill**
...
5. **Interaction state confined to one screen, more than two values?** → a Zustand slice **in the
   ...
   **screen store** holding view state only — `acme-state-management`'s composite rung and
...
- **Third-party** — the upstream system mock in-process, the style catalogue ingest, and any future partner
  calls all sit behind the proxy boundary (integration tests prove the seams — `acme-test`).
...
— read that first, and load `acme-security` for a review. `your contract package pathsrc/policy.ts` is the
...
with no framework types in their signatures — see `acme-api-contract`. A route handler that grew a
...
1024px tablet-landscape trap live in `acme-design` §2.4 and
