...
`design` owns how it looks. `component-development` owns how a *primitive* is built. This
...
3. **Decide what already exists.** Every element is one of: an existing `your component library` primitive, a *new
   ...
   work, and a new primitive is `component-development`'s job, not this file's.
...
route in `your app path` fetches it and passes it down — that seam is `app-development`. **Move
...
**`refactor` owns the loop these phases sit inside** — MEASURE → PIN → MOVE → PROVE → RECORD,
