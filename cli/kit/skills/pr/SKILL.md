...
**The separator is whether the work is new or existing.** A new feature or enhancement is a `feature`. A defect is a `bugfix`. Everything else — tooling, deps, CI, docs — is a `chore` or `infra`. `infra` is for AWS and pipeline work owned by your infrastructure package.

A bugfix with an unclear cause gets a `bugfix` change folder first (`bugfix` owns this). A feature touching two layers is two PRs.
...
`test` owns the runbook. CI is currently disabled (`CI` workflow is `disabled_manually`) — a label is a note to a human, not an automatic gate.
