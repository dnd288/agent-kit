---
name: bugfix
description: "The defect loop — inspect, debug, IMPACT, fix, verify. Load this when fixing any bug, investigating a failure, or deciding whether a defect needs a change folder at all. Also load it when a test is failing and the cause is not yet known, when a fix is ready and needs proving, or when writing report.md, diagnosis.md, flows.md or verification.md in a `bugfix` change. Owns the threshold that decides between a straight pull request and a bugfix change, the rule that the diagnosis carries its evidence, the impact pass that keeps a fix backward compatible, the run set a fix owes — the defect's own feature, the cross-cutting health slice, and whatever the impact named — and the RED-before-GREEN discipline. Routes to tdd for which test mode can observe which claim, test for bringing the stacks up, e2e for the feature file, your contribution guide for the pull-request rules, and your project architectural decisions for why the type exists."
---
...
`refactor`, whose defining constraint is the opposite of this loop's: behaviour does not change.
...
does today?** Grep for the callers rather than remembering them. `refactor`'s smell catalogue
...
| A share link | A **capability token**, not a credential. Change how one is issued or read and every link already sent stops working — `security` owns the surface |
...
`tdd` maps the claim to the mode that can observe it — geometry needs Chromium, copy needs
...
claim gets slow and vague, and `tdd` owns the mapping. Those are the only
...
is wrong. `e2e` owns the file, the layers under it and the placement question.
...
slice* is the reference and `test` owns bringing the stacks up.
...
- [ ] The change exists on the `bugfix` schema, or the cause was genuinely obvious and it is a plain pull request
