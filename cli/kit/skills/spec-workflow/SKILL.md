...
straight to a pull request. **A defect whose cause is not obvious gets a `bugfix` change** —
see `bugfix`.
...
| `bugfix` | a defect at any layer | inspect · debug · fix · verify | `features/defects/<symptom>.feature` |
...
application is *wrong* is a defect, and belongs to `bugfix`.
...
| **Flows** | `changes/<id>/flows.md` | — | `e2e` — the business logic, as Gherkin, before anything else |
...
| **Verify** | `changes/<id>/verification.md` | `your validation command`, then `cd e2e && your e2e test command <slice>` | `verify`, `review` |
...
- `verify` — the command that proves it. Run by the verify gate, not by the builder.
...
| `integration` | `flows` |
| `bugfix` | `test` · `fix` — in that order, and the order is the discipline |
...
happens to be first** — the same shape as `bugfix`'s `test` before `fix`, and for the same
reason. A `flows` task's `verify` is the selection it runs: `cd e2e && your e2e test command <split>/<name>`.

Several `build.json` files written before this vocabulary settled use names from an older one
(`ui`, `design`, `api`, `worker`, `infra`). They are not built by the workflow, and were not
...
- A task whose `verify` command is missing. It cannot be handed over.
