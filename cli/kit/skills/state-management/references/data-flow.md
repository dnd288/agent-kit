...
|---|---|---|---|
| **Store holds a database row** | a store with a `project`/`user`-shaped copy | the copy diverges from the source; the read was rule 1 | — (review; `state-management` ladder) |
...
| **Route holds business logic** | a page.tsx with transactions/branches | the value's home is a service, not a route | — (`api-contract`) |
| **Mutation without revalidate** | an update visible only after reload | the router cache serves the old payload — looks exactly like a caching bug | — (review; `app-development` §7d) |
